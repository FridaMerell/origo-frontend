...
import { ApsisDataProvider } from "./apsis-context";

export async function ApsisProviders({ children }: { children: ReactNode }) {
  const posts = [...(await getApsisPosts())].sort((a, b) => b.created_at.localeCompare(a.created_at));
  ...
  return <ApsisDataProvider posts={posts} users={users}>{children}</ApsisDataProvider>;
}
