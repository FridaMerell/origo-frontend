...
export type SplashTenant = "verso" | "flux" | "apsis" | "tempus" | "opus"
...
const CONFIG: Record<SplashTenant, TenantConfig> = {
	...
	opus: {
		wordmark: "Opus",
		caption: "Översätter från knittel …",
		sizeClass: "text-[2.5rem]",
		weightClass: "font-medium",
		trackingClass: "tracking-normal",
		captionClass: "text-text-muted",
...
function VersoSpinner() {
	return (
		<svg
			...
			aria-hidden
			style={{
				animation: "splash-spin 1.6s cubic-bezier(.4,0,.2,1) infinite",
			}}>
			<circle
				cx='32'
				cy='32'
			...
			<circle
				...
				style={{
					...
					animation:
						"splash-verso-breathe 1.6s cubic-bezier(.4,0,.2,1) infinite",
				}}
			/>
		</svg>
	)
		...
		<div
			...
			<div
				style={{
					...
					bottom: 32,
					animation:
						"splash-tempus-needle 2.4s cubic-bezier(.3,.6,.3,1) infinite",
				}}
			/>
...
export function Splash({ tenant }: { tenant: SplashTenant }) {
	...
	const wordmarkStyle: CSSProperties = {
		...
		...(config.wordmarkAnimation
			? { animation: config.wordmarkAnimation }
			: {}),
	}
	...
	return (
		<div
			...
			<p
				className={`absolute inset-x-0 bottom-14 text-center text-sm ${config.captionClass}`}>
				{config.caption}
			</p>
		</div>
	)
