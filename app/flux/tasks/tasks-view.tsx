...
function TaskRow({ task, projectName }: { task: FluxTask; projectName: string }) {
  ...
  return (
    <div onClick={() => openTask(task.id)} className={isTaskOverdue(task.due_date, task.status) ? "group flex cursor-pointer items-center gap-3 border-b border-danger/20 bg-danger-wash/30 px-4 py-3.5 last:border-b-0 hover:bg-danger-wash/50" : "group flex cursor-pointer items-center gap-3 border-b border-border px-4 py-3.5 last:border-b-0 hover:bg-surface-2"}>
...
function TaskSection({ title, hint, tasks, projectName, tone = "default" }: { title: string; hint: string; tasks: FluxTask[]; projectName: (id: number) => string; tone?: "default" | "danger" }) {
  ...
  return <section className="max-w-5xl"><div className="mb-3 flex items-baseline gap-3"><h2 className={tone === "danger" ? "text-base font-semibold text-danger" : "text-base font-semibold text-text"}>{title}</h2><span className="text-xs text-text-faint">{hint} · {tasks.length}</span></div><Card className="!gap-0 !p-0 overflow-hidden">{tasks.map((task) => <TaskRow key={task.id} task={task} projectName={projectName(task.project)} />)}</Card></section>
}
