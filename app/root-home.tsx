...
const APP_ICONS: Record<string, StaticImageData> = { apsis: apsisIcon, flux: fluxIcon, tempus: tempusIcon, verso: versoIcon }
...
const APP_DESCRIPTION: Record<string, string> = {
  verso: "Besökskalender, projekthantering, ekonomi och viktiga dokument för hus eller sommarstugor.",
  flux: "Projekthantering med milstolpar, uppgifter, dokumentation och deadlines.",
  tempus: "Taxonomisk data, säsongsbedömningar och biotoper för arter. Men även observationer, krysslistor, hjälp att planera utflykter och en ML-mottagare för automatisk klassificering av fågelsång.",
...
export default function RootHome() {
  ...
  useEffect(() => { setHrefs(Object.fromEntries(APP_LINKS.map((app) => [app.id, appHref(app.id)]))) }, [])
  ...
  const updatePreview = (id: string, x: number, y: number) => {
    ...
    const left = x + offset + width <= window.innerWidth - margin ? x + offset : x - offset - width
    const top = y + offset + height <= window.innerHeight - margin ? y + offset : y - offset - height
    setPreview({ id, x: Math.max(margin, Math.min(left, window.innerWidth - width - margin)), y: Math.max(margin, Math.min(top, window.innerHeight - height - margin)) })
  }

  return <main className="relative min-h-screen overflow-hidden bg-[#E7E5DE] px-5 py-6 text-[#1B252B] sm:px-10 sm:py-10">
    <span aria-hidden className="absolute left-1/2 top-0 h-full w-px bg-[#1B252B]/15" /><span aria-hidden className="absolute left-0 top-1/2 h-px w-full bg-[#1B252B]/15" />
    <div className="relative mx-auto flex min-h-[calc(100vh-3rem)] max-w-6xl flex-col">
      <header className="flex items-center justify-between"><span className="font-[family-name:var(--font-geist-mono)] text-[11px] font-medium uppercase tracking-[0.18em]">ORIGO / 00°00′</span>{user && <div className="flex items-center gap-4"><a href="/konto" className="font-[family-name:var(--font-geist-mono)] text-[11px] uppercase tracking-[0.12em] underline underline-offset-4">Konto</a><a href="/konto/anslutningar" className="font-[family-name:var(--font-geist-mono)] text-[11px] uppercase tracking-[0.12em] underline underline-offset-4">Anslutningar</a><button type="button" onClick={() => void logout().then(() => window.location.reload())} className="font-[family-name:var(--font-geist-mono)] text-[11px] uppercase tracking-[0.12em] underline underline-offset-4">Logga ut</button></div>}</header>
      <section className="flex flex-1 flex-col justify-center py-16">
        <div className="flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between">
        <div className="relative max-w-3xl bg-[#E7E5DE]/90 py-5">
          <span className="mb-5 flex size-12 items-center justify-center rounded-full border border-[#1B252B] font-[family-name:var(--font-geist-mono)] text-xs">0,0</span>
          <p className="font-[family-name:var(--font-geist-mono)] text-[11px] uppercase tracking-[0.18em] text-[#58636A]">Gemensam utgångspunkt</p>
          <h1 className="mt-3 text-5xl font-medium tracking-[-0.06em] sm:text-7xl">Ett inlogg.<br />Mycket dumt.</h1>
        </div>
        ...
        {previewApp && preview && <aside aria-hidden className="pointer-events-none fixed z-20 max-h-[calc(100dvh-2rem)] w-70 max-w-[calc(100vw-2rem)] overflow-y-auto border border-[#1B252B] bg-[#1B252B] p-4 text-[#F4F2EC] shadow-[7px_7px_0_rgba(27,37,43,0.25)]" style={{ left: preview.x, top: preview.y }}>
          ...
          <div className="relative mb-4 flex items-center justify-between border-b border-[#F4F2EC]/25 pb-3"><span className="font-[family-name:var(--font-geist-mono)] text-[10px] uppercase tracking-[0.16em]">Punkt 0{apps.indexOf(previewApp) + 1}</span><span className="font-[family-name:var(--font-geist-mono)] text-[10px] text-[#C9D0CE]">↗</span></div>
          {APP_SCREENSHOTS[previewApp.id] ? <Image src={APP_SCREENSHOTS[previewApp.id]} alt="" width={560} height={320} className="relative mb-4 aspect-video w-full border border-[#F4F2EC]/25 object-cover object-top" /> : <div className="relative mb-4 flex aspect-video items-center justify-center border border-[#F4F2EC]/25 bg-[#243239]"><span className="absolute left-1/2 top-0 h-full w-px bg-[#F4F2EC]/10" /><span className="absolute left-0 top-1/2 h-px w-full bg-[#F4F2EC]/10" /><Image src={APP_ICONS[previewApp.id]} alt="" width={76} height={76} className="relative size-16 object-contain" /><span className="absolute bottom-2 left-3 font-[family-name:var(--font-geist-mono)] text-[9px] uppercase tracking-[0.12em] text-[#C9D0CE]">Förhandsvisning saknas</span></div>}
          <div className="relative"><p className="font-[family-name:var(--font-geist-mono)] text-[10px] uppercase tracking-[0.16em] text-[#C9D0CE]">{previewApp.name}</p><p className="mt-2 text-sm leading-relaxed text-[#F4F2EC]/85">{APP_DESCRIPTION[previewApp.id]}</p></div>
        </aside>}
      </section>
      <footer className="font-[family-name:var(--font-geist-mono)] text-[10px] uppercase tracking-[0.16em] text-[#58636A]">Projektet är byggt i Django, med Next.js som frontend.<br/>Kontakt: frida.merell@gmail.com</footer>
    </div>
  </main>
}
